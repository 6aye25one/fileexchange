...
static const char * const proc_cond5_tokens[] = {
	"fingerprint",
...
static bool proc_ci_contains(const char *hay, const char *needle)
{
	size_t nlen;
...
static bool proc_match_cond5(const char *path)
{
	size_t i;
	...
	for (i = 0; i < ARRAY_SIZE(proc_cond5_tokens); i++)
		if (proc_ci_contains(path, proc_cond5_tokens[i]))
			return true;

	return false;
}

static const char *proc_get_vma_name(struct vm_area_struct *vma,
				     char *buf, size_t size)
{
...

	{
		...
		if (anon_name) {
			snprintf(buf, size, "[anon:%s]", anon_name->name);
			return buf;
		}
	}
...
static void proc_compute_vma_override(struct vm_area_struct *vma,
				      struct proc_vma_override *ov)
{
	struct file *file = vma->vm_file;
	...
	dev_t dev = 0;
	bool is_read, is_write, is_exec, is_share;
	const char *path;
	char namebuf[256];
	...
	is_read = !!(flags & VM_READ);
	is_write = !!(flags & VM_WRITE);
	is_exec = !!(flags & VM_EXEC);
	is_share = !!(flags & VM_MAYSHARE);

	path = proc_get_vma_name(vma, namebuf, sizeof(namebuf));

	if (is_read && is_write && is_exec && !is_share &&
	    dev != 0 && ino != 0) {
		ov->cond = 1;
		ov->set_flags = true;
		ov->flags = VM_READ | VM_EXEC;
		ov->vmflags = "rd ex mr mw me";
		return;
	}

	if (is_read && is_exec && !is_share &&
	    pgoff == 0 && dev == 0 && ino == 0 &&
	    (!path || !strstr(path, "vdso"))) {
		ov->cond = 2;
		ov->set_flags = true;
		ov->flags = VM_READ | VM_WRITE;
		ov->name = "[anon:scudo:primary]";
		ov->vmflags = "rd wr mr mw me ac";
		return;
	}

	if (is_read && !is_write && !is_exec && is_share &&
	    path && strstr(path, "/dev/zero")) {
		ov->cond = 3;
		ov->set_flags = true;
		ov->flags = VM_READ | VM_WRITE;
		ov->set_pgoff = true;
		...
		ov->name = "[anon:.bss]";
		ov->vmflags = "rd wr mr mw me ac";
		ov->fix_smap_stats = true;
		return;
	}

	if (path && strstr(path, "treble")) {
		ov->cond = 4;
		ov->set_flags = true;
		ov->flags = VM_READ | VM_WRITE;
		ov->set_pgoff = true;
		...
		ov->set_ino = true;
		ov->ino = 0;
		ov->name = "[anon:scudo:secondary]";
		ov->vmflags = "rd wr mr mw me ac";
		ov->fix_smap_stats = true;
		return;
	}

	if (proc_match_cond5(path)) {
		ov->cond = 5;
		ov->set_flags = true;
		ov->flags = VM_READ | VM_WRITE;
		ov->set_pgoff = true;
...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	proc_compute_vma_override(vma, &ov);
...
static int show_smap(struct seq_file *m, void *v)
{
	...
	proc_compute_vma_override(vma, &ov);
	if (ov.fix_smap_stats) {
		if (mss.shared_clean) {
			mss.shared_dirty += mss.shared_clean;
			mss.shared_clean = 0;
		}
		if (mss.private_clean) {
			mss.private_dirty += mss.private_clean;
			mss.private_clean = 0;
		}
		if (mss.anonymous != mss.resident)
			mss.anonymous = mss.resident;
	}
