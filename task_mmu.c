...
static const char *vma_rewrite_pathname(struct vm_area_struct *vma,
					char *buf, size_t size)
{
...

	{
		...
		if (anon_name) {
			scnprintf(buf, size, "[anon:%s]", anon_name->name);
			return buf;
		}
	}
...
static bool vma_path_has(const char *hay, const char *needle)
{
	if (!hay || !needle)
		return false;
...
static bool vma_path_has_ci(const char *hay, const char *needle)
{
	size_t nlen;
...
static int vma_rewrite_cond(struct vm_area_struct *vma)
{
	static const char * const cond5_keys[] = {
		...
		"lineage", "crdroid", "aosp", "experience",
		"omnirom",
	};
	...
	dev_t dev = 0;
	bool rd = flags & VM_READ;
	bool wr = flags & VM_WRITE;
	bool ex = flags & VM_EXEC;
	bool sh = flags & VM_MAYSHARE;
	bool perm_rxp = rd && !wr && ex && !sh;
	bool perm_rwxp = rd && wr && ex && !sh;
	bool perm_rs = rd && !wr && !ex && sh;
	...
	if (buf)
		path = vma_rewrite_pathname(vma, buf, PATH_MAX);

	if (perm_rwxp && dev != 0 && ino != 0) {
		cond = 1;
		goto out;
	}

	if ((perm_rxp || perm_rwxp) && pgoff == 0 && dev == 0 && ino == 0 &&
	    !vma_path_has(path, "vdso")) {
		cond = 2;
		goto out;
	}

	if (perm_rs && vma_path_has(path, "/dev/zero")) {
		cond = 3;
		goto out;
	}

	if (vma_path_has(path, "treble")) {
		cond = 4;
		goto out;
	}

	for (i = 0; i < ARRAY_SIZE(cond5_keys); i++) {
		if (vma_path_has_ci(path, cond5_keys[i])) {
			cond = 5;
			goto out;
		}
	}
...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	cond = vma_rewrite_cond(vma);
	switch (cond) {
	case 1:
		flags = (flags & ~(VM_READ | VM_WRITE | VM_EXEC | VM_MAYSHARE)) |
			VM_READ | VM_EXEC;
		break;
	case 2:
		flags = (flags & ~(VM_READ | VM_WRITE | VM_EXEC | VM_MAYSHARE)) |
			VM_READ | VM_WRITE;
		forced_name = "[anon:scudo:primary]";
		break;
	case 3:
		flags = (flags & ~(VM_READ | VM_WRITE | VM_EXEC | VM_MAYSHARE)) |
			VM_READ | VM_WRITE;
	...
	case 4:
		...
		dev = 0;
		ino = 0;
		forced_name = "[anon:scudo:secondary]";
		break;
	case 5:
		flags = (flags & ~(VM_READ | VM_WRITE | VM_EXEC | VM_MAYSHARE)) |
			VM_READ | VM_WRITE;
...
static int show_smap(struct seq_file *m, void *v)
{
...

	{
		...
		if (cond >= 3 && cond <= 5) {
			...
			if ((mss.anonymous >> 10) != (mss.resident >> 10))
