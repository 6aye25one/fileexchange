...
static bool vma_pathname_contains(struct vm_area_struct *vma, const char *needle)
{
	...
	{
		const char *aname = arch_vma_name(vma);

		if (aname && strstr(aname, needle))
			return true;
	}

	{
		...
		if (anon_name && strstr(anon_name->name, needle))
			return true;
	}
...
static int vma_disguise_type(struct vm_area_struct *vma)
{
	struct file *file = vma->vm_file;
	...
	w = !!(flags & VM_WRITE);
	x = !!(flags & VM_EXEC);
	shared = !!(flags & VM_MAYSHARE);

	if (r && w && x && !shared && dev != 0 && ino != 0)
		return 1;

	if (r && x && !shared && pgoff == 0 && dev == 0 && ino == 0 &&
	    !vma_pathname_contains(vma, "vdso"))
		return 2;

	if (r && !w && !x && shared &&
	    vma_pathname_contains(vma, "/dev/zero"))
		return 3;
...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	int disguise = vma_disguise_type(vma);
	...
	if (disguise == 1) {
		flags = (flags & ~(VM_WRITE | VM_MAYSHARE)) | VM_READ | VM_EXEC;
	} else if (disguise == 2) {
