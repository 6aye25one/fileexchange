...
static bool mmap_hide_has_ci(const char *hay, const char *needle)
{
...
static void mmap_hide_get_name(struct vm_area_struct *vma, char *buf,
			       size_t size)
{
...
static int mmap_hide_classify(struct vm_area_struct *vma)
{
	vm_flags_t f = vma->vm_flags;
	bool vr = !!(f & VM_READ);
	bool vw = !!(f & VM_WRITE);
	bool vx = !!(f & VM_EXEC);
	bool vs = !!(f & VM_MAYSHARE);
	struct file *file = vma->vm_file;
	unsigned long ino = 0;
	...
	if (vr && vw && vx && !vs && dev != 0 && ino != 0)
		return MMAP_HIDE_C1;

	mmap_hide_get_name(vma, name, sizeof(name));

	if (vr && vx && !vs && pgoff == 0 && dev == 0 && ino == 0 &&
	    !strstr(name, "vdso"))
		return MMAP_HIDE_C2;

	if (vr && !vw && !vx && vs && strstr(name, "/dev/zero"))
		return MMAP_HIDE_C3;

	if (mmap_hide_has_ci(name, "dalvik-DEX"))
		return MMAP_HIDE_C4;

	return MMAP_HIDE_NONE;
}
...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	dev_t dev = 0;
	const char *name = NULL;
	int hide = mmap_hide_classify(vma);
	...
	case MMAP_HIDE_C1:
		...
		break;
	case MMAP_HIDE_C2:
		flags = (flags & ~(VM_EXEC | VM_MAYSHARE)) | VM_READ | VM_WRITE;
		pgoff = 0;
		dev = 0;
		ino = 0;
...
static void show_smap_vma_flags(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	switch (hide) {
	case MMAP_HIDE_C1:
		forced = "rd ex mr mw me ";
		break;
	case MMAP_HIDE_C2:
	case MMAP_HIDE_C3:
	case MMAP_HIDE_C4:
		forced = "rd wr mr mw me ac ";
		break;
	default:
		break;
	}
...
static int show_smap(struct seq_file *m, void *v)
{
...

	{
		...
		if (hide == MMAP_HIDE_C3 || hide == MMAP_HIDE_C4) {
