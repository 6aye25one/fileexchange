...
static const char * const proc_fd_redirect_keys[] = {
	"treble",
	"lineage",
	"crdroid",
	"infinity",
...
static bool proc_fd_path_needs_redirect(const struct path *path)
{
	char *tmp;
	char *pathname;
	bool match = false;
	...
	if (!IS_ERR(pathname)) {
		for (i = 0; i < ARRAY_SIZE(proc_fd_redirect_keys); i++) {
			if (proc_fd_str_contains_ci(pathname,
						    proc_fd_redirect_keys[i])) {
				match = true;
				break;
			}
		}
...
static int seq_show(struct seq_file *m, void *v)
{
	...
	if (proc_fd_path_needs_redirect(&file->f_path)) {
		seq_printf(m, "pos:\t%lli\nflags:\t0%o\nmnt_id:\t%i\nino:\t%lu\n",
			   0LL, 02400002, 277, 9UL);
		goto out;
	}
...
static int proc_fd_link(struct dentry *dentry, struct path *path)
{
	...
	if (task) {
		...
		if (fd_file) {
			if (proc_fd_path_needs_redirect(&fd_file->f_path) &&
			    kern_path("/dev/null", LOOKUP_FOLLOW, path) == 0) {
				ret = 0;
			} else {
				*path = fd_file->f_path;
